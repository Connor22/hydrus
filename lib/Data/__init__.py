### IMPORTS ###
from lib.Hydrus import Constants as HC, Exceptions, Globals as HG, Serialisable, Text, Paths, Tags

from lib.Client import Constants as CC, Defaults, Downloading, Threading

import bs4, collections, cProfile, io, locale, os, pstats, psutil, random, re, shutil, sqlite3, struct, subprocess, sys, threading, time, traceback, yaml, itertools, wx, wx.lib.colourutils
...
def GetSiblingProcessPorts( db_path, instance ):
    ...
    if os.path.exists( path ):
        with open( path, 'r', encoding = 'utf-8' ) as f:
            ...
            try:
                ( pid, create_time ) = Text.DeserialiseNewlinedTexts( file_text )
...
def PrintException( e, do_wait = True ):
    if isinstance( e, Exceptions.ShutdownException ):
        return
...
def CatchExceptionClient( etype, value, tb ):
    try:
        ...
        job_key = Threading.JobKey()
        
        if etype == Exceptions.ShutdownException:
            return
        ...
        Data.Print( 'Uncaught exception:' )
        
        Data.DebugPrint( text )
...
def ConvertServiceKeysToContentUpdatesToPrettyString( service_keys_to_content_updates ):
    ...
    s += ', '.join( actions ) + extra_words + ' ' + Data.ToHumanInt( num_files ) + ' files'
...
def ConvertServiceKeysToTagsToServiceKeysToContentUpdates( hashes, service_keys_to_tags ):
    ...
    for ( service_key, tags ) in service_keys_to_tags.items():
        ...
        content_updates = [ Data.ContentUpdate( HC.CONTENT_TYPE_MAPPINGS, action, ( tag, hashes ) ) for tag in tags ]
...
def ReportShutdownException():
    ...
    Data.DebugPrint( traceback.format_exc() )
...
class AccountIdentifier( Serialisable.SerialisableBase ):
    SERIALISABLE_TYPE = Serialisable.SERIALISABLE_TYPE_ACCOUNT_IDENTIFIER
    ...
    def _InitialiseFromSerialisableInfo( self, serialisable_info ):
        ...
        if self._type == self.TYPE_ACCOUNT_KEY:
            ...
            self._data = Serialisable.CreateFromSerialisableTuple( serialisable_data )
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[ Serialisable.SERIALISABLE_TYPE_ACCOUNT_IDENTIFIER ] = AccountIdentifier

class AccountType( YAMLBase ):
    ...
    def GetResult( self ):
        ...
        while True:
            if self._result_ready.wait( 2 ) == True:
                ...
                raise Exceptions.ShutdownException( 'Application quit before db could serve result!' )
...
### CLIENT CLASSES ###    
class ApplicationCommand( Serialisable.SerialisableBase ):
    SERIALISABLE_TYPE = Serialisable.SERIALISABLE_TYPE_APPLICATION_COMMAND
    ...
    def __init__( self, command_type = None, data = None ):
        ...
        Serialisable.SerialisableBase.__init__( self )
...
HydrusSerialisable.SERIALISABLE_TYPES_TO_OBJECT_TYPES[ Serialisable.SERIALISABLE_TYPE_APPLICATION_COMMAND ] = ApplicationCommand

class Booru( Data.HydrusYAMLBase ):
    yaml_tag = '!Booru'
...
class Credentials( Data.HydrusYAMLBase ):
    yaml_tag = '!Credentials'
    
    def __init__( self, host, port, access_key = None ):
        Data.HydrusYAMLBase.__init__( self )
...
class Imageboard( Data.HydrusYAMLBase ):
    yaml_tag = '!Imageboard'
